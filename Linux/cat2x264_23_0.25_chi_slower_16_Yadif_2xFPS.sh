...
input="$1/$2"
outputdir=/home/rzcat/x264/output
output=$(basename $1)
echo
echo Input: 
...
if [ $SubNum -gt 0 ]; then
	tail=\(x264cmb_${AudioNum}Aac_${SubNum}Sub.${videoencq}_${audioencq}.${speedctl}\)
else
	tail=\(x264cmb_${AudioNum}Aac.${videoencq}_${audioencq}.${speedctl}\)
fi
