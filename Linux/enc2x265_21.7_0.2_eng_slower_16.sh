...
function compress () {
	...
	if [ $(ffmpeg-10bit -i "$input" 2>&1 | grep "Subtitle" | wc -l) -gt 0 ]; then
		tail=\(x265_$(ffmpeg-10bit -i "$input" 2>&1 | grep "Audio" | wc -l)Aac_$(ffmpeg-10bit -i "$input" 2>&1 | grep "Subtitle" | wc -l)Sub.${videoencq}_${audioencq}.${speedctl}\)
	else
		tail=\(x265_$(ffmpeg-10bit -i "$input" 2>&1 | grep "Audio" | wc -l)Aac.${videoencq}_${audioencq}.${speedctl}\)
	fi
